...
const { ArrayEncoding } = require('./encoding')
...
File.prototype.open = function (size, cb) {
	fs.open(_path.join(__dirname, '../db', this.path), 'r+', (err, fd) => {
		...
		fs.read(this.fd, fileBuffer, 0, size, 0, (err, bytesRead, buffer) => {
			this.size = ArrayEncoding.decode(fileBuffer, size, this.buffer)
			this.ready(cb)
		})
	})
}
...
File.prototype.load = function (chunk) {
	...
	if (!this.fd) {
		...
		const chunk_ = new Float64Array(returnBuffer)
		chunk_[X] = chunk[X]
		chunk_[Y] = chunk[Y]
		return returnBuffer
	}
	...
	buttons.writeDoubleLE(chunk[X], 0)
	buttons.writeDoubleLE(chunk[Y], 8)
...
File.prototype.press = function (press) {
	...
	const longBit = longPress ? 1 : 0

	const buffer = new ArrayBuffer(16 + 2)
	const data = new DataView(buffer)

	data.setFloat64(0, press.chunk[X], true)
	data.setFloat64(8, press.chunk[Y], true)

	const bytes = press.i | (longBit << 15)
